...
const TransferPage = () => {
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await apiClient.post('/saldo/transfer', { no_hp: noHp, jumlah, pin, catatan });
    ...
    <div className="min-h-screen bg-gray-200 flex justify-center">
      <div className="max-w-[375px] w-full bg-white min-h-screen shadow-lg relative">
        ...
        <form onSubmit={handleTransfer} className="p-6 space-y-4">
          <input type="text" placeholder="Nomor HP Penerima" className="border rounded-lg p-3 w-full" value={noHp} onChange={(e) => setNoHp(e.target.value)} required />
          ...
          <input type="text" placeholder="Catatan (opsional)" className="border rounded-lg p-3 w-full" value={catatan} onChange={(e) => setCatatan(e.target.value)} />
